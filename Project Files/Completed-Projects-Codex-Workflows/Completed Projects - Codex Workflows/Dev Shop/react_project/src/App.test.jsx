...
describe('Add to cart', () => {
  ...
  it('shows correct line total for a product with quantity 2', async () => {
    ...
    await user.click(addButton);
    await user.click(addButton);
    // Mechanical Keyboard $89.99 × 2 = $179.98 (line total and original total are both $179.98)
  ...
  it('does not show a discount when the cart total is $100 or less', async () => {
    const { user } = setup();
    await user.click(screen.getAllByRole('button', { name: /add to cart/i })[0]);
  ...
  it('applies a 10% discount when the cart total exceeds $100', async () => {
    const { user } = setup();
    const addButtons = screen.getAllByRole('button', { name: /add to cart/i });
    await user.click(addButtons[0]);
    await user.click(addButtons[1]);
    ...
    expect(screen.getByText('$112.48')).toBeInTheDocument();
  });
});
